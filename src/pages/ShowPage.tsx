...
export const ShowPage: React.FC = () => {
  ...
  if (!show || !show.streamingOptions?.es) {
    return <div>Cargando...</div> //TODO: Añadir loading o fallback
  }
  ...
  return (
    <div className='relative h-screen w-screen overflow-hidden'>
      <div
        ...
        style={{
          backgroundImage: `url(${show?.imageSet.horizontalBackdrop.w1440 || show?.imageSet.horizontalPoster.w1440})`,
        }}
      >
      ...
      <section className='relative z-10 flex h-full max-w-[1600px] flex-col items-start justify-center px-4 text-center text-white sm:px-6 lg:px-8 xl:px-12'>
        <div>
          <h1 className='mb-10 w-full max-w-[650px] text-pretty text-start text-3xl font-bold md:mb-1 md:text-6xl'>
            {show?.title}
          </h1>
          <div className='my-5 flex flex-wrap gap-1 md:gap-2 lg:gap-4'>
          ...
          <p className='w-full max-w-[650px] text-pretty text-start text-gray-100 md:block md:text-lg lg:text-xl'>
            {show?.overview ? show.overview : show?.title}
          </p>

          <h2 className='my-4 text-start text-lg'>You can watch on:</h2>
